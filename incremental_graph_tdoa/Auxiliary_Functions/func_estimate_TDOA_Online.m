function [TDOA_est_mtx,CPSD] = func_estimate_TDOA_Online(Y_block,CPSD,lambda,fs,NIFFT,STFT_upsample,TDOA_Limits,num_candidates,TDOA_Method)
M = size(Y_block,1);
K = NIFFT/STFT_upsample;

% Taper lowest and highest frequencies to 0.
freqz = permute(fs*((1:(K/2+1))-1)/K,[1,3,2,4]);
w = ones(size(freqz));
...
CPSD = lambda*CPSD + (1-lambda)*Y_block.*permute(conj(Y_block),[2,1,3]);% .* Weight(:,:,:,ll);
...
GCC_PHAT = ifftshift(  ifft(cat(3,CPSD_PHAT, ...
    zeros(M,M,NIFFT-K,1), ...%zeros(M,length(Ref_ind),NFFT-K,1), ...
    conj(CPSD_PHAT(:,:,(end-1):-1:2))), NIFFT, 3,'symmetric') ,3);
GCC_PHAT = GCC_PHAT*NIFFT/(sum(w) + sum(w(1,1,2:(end-1)))); % undo scaling of tapering and zero-padding

lags_3D =  reshape(( (0:(NIFFT-1)) - NIFFT/2) / STFT_upsample / fs , 1, 1, []);
select_TDOAs = abs(lags_3D) <= (TDOA_Limits(:,:));
...
[TDOA_est_mtx, Reliability_1, ~] = func_Optimized_Findpeaks3_(GCC_PHAT, lags_3D, TDOA_Limits, num_candidates);
...
switch TDOA_Method
    ...
    case {'R'}
        % Reliability_1((1:(M+1):(M^2)) + ((1:num_candidates) -1)' * M^2 ) = Inf;
    ...
    case {'MST+'}
        ...
        TDOA_est_mtx = func_MSTplus(Reliability_1,CPSD,GCC_PHAT,lags_3D,M,fs,TDOA_Limits,select_TDOAs,w);
end
TDOA_est_mtx((1:(M+1):(M^2)) + ((1:num_candidates) -1)' * M^2 ) = 0;
end

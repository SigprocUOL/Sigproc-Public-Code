...
[x_t,fs] = audioread(fullfile('Data',[Filename '.wav'])); % [signal, sampling frequency [Hz]]
...
y_t = func_AddNoise(x_t, SNR_dB, M_mtx, M, fs, c);
...
[GCC_Resampling, NIFFT] = func_Compute_GCC_Resampling_from_Frame_Length(K,fs); % This function automatically determines some GCC-PHAT upsampling factors (using zero-padding of CPSDs) based on frame length.
...
if assume_mic_geometry_known_yn
    ...
    TDOA_Limits = D/c + 1/(fs*GCC_Resampling); % Geometric limits for minimum and maximum possible TDOAs
else
    TDOA_Limits = Inf(M,M); %#ok<UNRCH>
end

[lambda,~] = func_Compute_lambda(Smoothing_time,F,fs); % input: Smoothing_time, Frame_shift, fs

num_TDOA_candidates = 1; % 1, ..., 3 or so
max_num_sources = 1;
...
% Method selection
disp(['Processing ' num2str(round(size(x_t,1)/fs,2)) ' s signal.'])
for Implementation_idx = 1:length(Implementations)
    ...
    while signal_indices(1) < size(y_t,1) 
        ...
        tic;
        [Y] = calc_STFT(y_frame,fs,K,K/F,Ana_window,K); % (signal,fs,nfft,overlap_factor,wType,NFFT)
        ...
        [TDOA_est_mtx,CPSD] = func_estimate_TDOA_Online( ... % Online TDOA estimation function
            ...
            lambda,fs,NIFFT,GCC_Resampling,TDOA_Limits,num_TDOA_candidates,Implementation);
        %% ----------------------------------------------------------------
        %% END OF PROCESSING ----------------------------------------------
        %% ----------------------------------------------------------------
...
[VAD_idxs] = func_Compute_VAD_idx(x_t,M,fs,K,F,Ana_window,K,false);
...
for Implementation_idx = 1:length(Implementations)
    for plot_idx = 1:M
        ...
        if plot_idx == 1
            plot(((1:size(x_t,1))-1)/fs, y_t(:,1),'r')
            hold on
            plot(((1:size(x_t,1))-1)/fs, x_t(:,1),'g')
            hold off
            ...
            if Implementation_idx == 1
                ...
                xlim([1,frames])
                ylim(TDOA_Limits(plot_idx,1)*[-1.2,1.2])
            end
            hold on;
            ...
            if Implementation_idx == length(Implementations)
                ...
                func_Compute_VAD_idx(x_t,M,fs,K,F,Ana_window,K,true,ylim);
            end
            if plot_idx == 2
    ...
    TDOA_Errors = abs(TDOA_true(2:end,:,:,:) - TDOA_est_mtx_full{Implementation_idx}(2:end,1,1,:)); % omit first microphone pair (1,1), because TDOA is 0 anyway.
    TDOA_Err(Implementation_idx,1) = mean( TDOA_Errors , [1,4]);
    TDOA_Acc(Implementation_idx,1) = sum(TDOA_Errors(2:end,:,:,:) < Acc_Threshold,'all')/numel(TDOA_Errors(2:end,:,:,:)); % omit first microphone pair (1,1), because TDOA is 0 anyway.
end

disp(['Results for ' Scenario ' (' list{indx} '): '])
disp(array2table([round(Processing_time_method,2),round(TDOA_Err*1e3,3),TDOA_Acc*100], ...
    'VariableNames', {'Processing time [s]','TDOA Estimation Error [ms]',...
    ['TDOA Estimation Accuracy [%] (percentage of errors within ' num2str(Acc_Threshold*1e3) ' ms)']}, ...

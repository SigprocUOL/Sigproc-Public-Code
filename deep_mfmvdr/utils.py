...
def vector_to_Hermitian(vec):
    ...
    mat[(...,) + triu + (np.zeros(triu[0].shape[0]),)
        ] = vec[..., :triu[0].shape[0]]
    start = triu[0].shape[0]
    mat[(...,) + tril + (np.zeros(tril[0].shape[0]),)
        ] = mat[(...,) + triu2 + (np.zeros(triu2[0].shape[0]),)]

    # imaginary component
    mat[(...,) + triu2 + (np.ones(triu2[0].shape[0]),)
        ] = vec[..., start:start + triu2[0].shape[0]]
    mat[(...,) + tril + (np.ones(tril[0].shape[0]),)] = - \
        mat[(...,) + triu2 + (np.ones(triu2[0].shape[0]),)]
...
def complex_solve_matrix_vector(A, b):
    ...
    A_big = torch.cat(
        (torch.cat((A[..., 0], -A[..., 1]),
                   dim=-1), torch.cat((A[..., 1], A[..., 0]), dim=-1)),
        dim=-2)
    b_big = torch.cat((b[..., 0], b[..., 1]), dim=-1)
    x_big = torch.solve(b_big[..., None],
                        A_big)[0][..., 0]  # ignore singleton dimension
...
def complex_solve_matrix_matrix(mat1, mat2):
    ...
    n = mat1.shape[-2]
    mat1_big = torch.cat(
        (torch.cat((mat1[..., 0], -mat1[..., 1]),
                   dim=-1), torch.cat((mat1[..., 1], mat1[..., 0]), dim=-1)),
        dim=-2)
    mat2_big = torch.cat((mat2[..., 0], mat2[..., 1]), dim=-2)
    solution_big = torch.solve(mat2_big, mat1_big)[0]
    return torch.stack((solution_big[..., :n, :], solution_big[..., n:, :]),
                       dim=-1)
...
def filter_minimum_gain_like(G_min, w, y, alpha=None, k=10.):
    """
...
def minimum_gain_like(G_min, Y, filtered_input, alpha=None, k=10.):
    if alpha is None:
        alpha = 1. / (1. + torch.exp(-2 * k *
                                     (complex_tensor_abs(filtered_input) -
                                      complex_tensor_abs(G_min * Y))))
        alpha = alpha[..., None]
        return_alpha = True
    else:
        return_alpha = False
...
@torch.jit.script
def complex_tensor_hadamard_product(a: torch.Tensor,
...
@torch.jit.script
def complex_tensor_matrix_vector_product(matrix: torch.Tensor,
...
def tik_reg(mat:torch.Tensor, reg: float = 0.001) -> torch.Tensor:
    ...
    temp = ((reg * trace(complex_tensor_abs(mat))) / iAdj)[..., None, None] * \
        torch.eye(iAdj, device=mat.device)[None, None, ...]
...
def complex_tensor_division(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    ...
    :return:
    """
    factor = 1. / (b[..., 0]**2 + b[..., 1]**2)
    result = torch.stack((a[..., 0] * b[..., 0] + a[..., 1] * b[..., 1],
                          a[..., 1] * b[..., 0] - a[..., 0] * b[..., 1]),
                         dim=-1)
...
def complex_tensor_inner_product(x, y):
    ...
    xHy[..., 0] = torch.sum(x[..., 0] * y[..., 0] + x[..., 1] * y[..., 1],
                            dim=-1)
    xHy[..., 1] = torch.sum(x[..., 0] * y[..., 1] - x[..., 1] * y[..., 0],
                            dim=-1)
    return xHy
